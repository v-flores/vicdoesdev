...
The most commonly discussed positives to headless web development architecture that I discovered included
- greater performance
...
While many developers might not be the biggest fans of Wordpress, there's no doubt most content managers and creators still love it. Headless CMS gives these professionals the ability to keep working in a platform they feel most comfortable with while allowing developers the opportunity to be free of all the conventions and limitations in place by these platforms. Developers can pipe the content of the CMS in more directions then before via a web-service API. The end-user experience gets delivered by a modern JavaScript framework. I considered possibly decoupling a previous Wordpress side project with Angular which I'm most familiar with. I instead ended up diving into the usage of a static site generator and the implementation of the Jamstack.
